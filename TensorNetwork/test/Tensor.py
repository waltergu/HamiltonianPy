...
def test_tensor_contract():
    ...
    t1=time()
    temp1=contract(mps[N/2-1:N/2+1],engine='einsum')
    print 'einsum time: %ss'%(time()-t1)
    t2=time()
    temp2=contract(mps[N/2-1:N/2+1],engine='tensordot')
    print 'tensordot time: %ss.'%(time()-t2)
    t3=time()
    temp3=contract(mps[N/2-1:N/2+1],engine='block',signs=['++-','++-'])
    print 'block time: %ss.'%(time()-t3)
    print 'diff: %s,%s,%s'%(norm(temp1-temp2),norm(temp2-temp3),norm(temp3-temp1))
