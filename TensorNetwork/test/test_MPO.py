...
class TestMPOBase(TestCase):
    def setUp(self):
        ...
        self.mpo=MPO.fromoperators(self.generator.operators,self.degfres,layer=self.layer,mode=self.mode)
...
class SpinBase(object):
    def init(self):
        ...
        self.degfres=DegFreTree(mode='QN',layers=self.layers,priority=self.priority,leaves=self.config.table(mask=[]).keys(),map=lambda index:SQNS(S))
        ...
        self.mps1=MPS.random(sites,bonds,mode=self.mode,cut=cut,nmax=20)
        self.mps2=MPS.random(sites,bonds,mode=self.mode,cut=cut,nmax=20)
...
class FermiBase(object):
    def init(self):
        ...
        self.degfres=DegFreTree(mode='QN',layers=self.layers,priority=self.priority,leaves=self.config.table(mask=['nambu']).keys(),map=lambda index:PQNS(1))
