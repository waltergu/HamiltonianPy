...
class SpinBase(object):
    def init(self):
        ...
        self.degfres=DegFreTree(layers=self.layers,leaves=self.config.table(mask=[]),map=lambda index:SQNS(S))
...
class FermiBase(object):
    def init(self):
        ...
        self.degfres=DegFreTree(layers=self.layers,leaves=self.config.table(mask=['nambu']),map=lambda index:PQNS(1))
