...
class TestDegFreTree(TestCase):
    ...
    def test_fermi_nb(self):
        print
        degmap=lambda index: 2
        tree=DegFreTree(mode='NB',layers=self.flayers,priority=self.fpriority,leaves=self.fconfig.table(mask=['nambu']).keys(),map=degmap)
    ...
    def test_fermi_qn(self):
        print
        degmap=lambda index: SzPQNS(-0.5 if index.spin==0 else 0.5)
        tree=DegFreTree(mode='QN',layers=self.flayers,priority=self.fpriority,leaves=self.fconfig.table(mask=['nambu']).keys(),map=degmap)
        for layer in self.flayers:
            print 'layer: %s'%(layer,)
            for i,index in enumerate(tree.indices(layer)):
    ...
    def test_spin_nb(self):
        print
        degmap=lambda index: 2
        tree=DegFreTree(mode='NB',layers=self.slayers,priority=self.spriority,leaves=self.sconfig.table(mask=[]).keys(),map=degmap)
    ...
    def test_spin_qn(self):
        print
        degmap=lambda index: SQNS(0.5)
        tree=DegFreTree(mode='QN',layers=self.slayers,priority=self.spriority,leaves=self.sconfig.table(mask=[]).keys(),map=degmap)
        for layer in self.slayers:
            print 'layer: %s'%(layer,)
            for i,index in enumerate(tree.indices(layer)):
                print 'i,index,len(tree[index]),tree[index]: %s, %s, %s, %s'%(i,repr(index),len(tree[index]),tree[index])
