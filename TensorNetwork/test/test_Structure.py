...
class TestDegFreTree(TestCase):
    ...
    def test_fermi_int(self):
        ...
        tree=DegFreTree(layers=self.flayers,priority=self.fpriority,leaves=self.fconfig.table(mask=['nambu']).keys(),map=degmap)
    ...
    def test_spin_int(self):
        print
        degmap=lambda index: 2
        tree=DegFreTree(layers=self.slayers,priority=self.spriority,leaves=self.sconfig.table(mask=[]).keys(),map=degmap)
