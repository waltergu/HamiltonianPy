...
def test_vca():
    ...
    a1=array([1.0,0.0])
    a2=array([0.0,1.0])
    points=tiling(cluster=[p1],vectors=[a1,a2],indices=itertools.product(xrange(m),xrange(n)),scope='WG'+str(m)+str(n),flatten_site=True)
    ...
    a=VCA(
            ...
            nspin=      2,
            cell=       Lattice(name='WG'+str(m)+str(n),points=[points[0]],vectors=[a1,a2]),
            lattice=    Lattice(name='WG'+str(m)+str(n),points=points,vectors=[a1*m,a2*n]),
            config=     config,
            ...
            nambu=      False,
            weiss=[     Onsite('afm',0.0,indexpackages=sigmaz('sp'),amplitude=lambda bond: 1 if bond.spoint.pid.site[2] in (0,3) else -1,modulate=lambda **karg:karg['afm'])]
            )
