...
def dmrg(system):
    return  [
            ...
            "    nspb,nmax=",
            "    if core=='idmrg':",
            "        tsg=DMRG.TSG(name='GROWTH',targets=targets,nspb=nspb,nmax=nmax,run=DMRG.DMRGTSG)",
            "        dmrg.register(tsg)",
            "    elif core=='fdmrg':",
            "        tsg=DMRG.TSG(name='GROWTH',targets=targets,nspb=nspb,nmax=nmax,plot=False,run=DMRG.DMRGTSG)",
            "        tss=DMRG.TSS(name='SWEEP',target=targets[-1],nsite=nspb*len(targets)*2,nmaxs=[nmax,nmax],dependences=[tsg],run=DMRGTSS)",
            "        dmrg.register(tss)",
            "    else:",
