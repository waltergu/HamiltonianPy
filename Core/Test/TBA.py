...
def test_tba():
    ...
    a=TBA(
        ...
        terms=[     Hopping('t1',-1.0),
                    Hopping('t2',-0.5,amplitude=lambda bond: 1 if (bond.spoint.pid.site[1]%2==1 and bond.rcoord[0]>0) or (bond.spoint.pid.site[1]%2==0 and bond.rcoord[0]<0) else -1),
    ...
    #a.addapps('EB',EB(path=BaseSpace({'tag':'mu','mesh':linspace(-3,3,num=201)}),run=TBAEB,save_data=False,plot=False))
    a.runapps()
